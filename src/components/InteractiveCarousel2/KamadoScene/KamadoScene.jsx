...
function PulseRing({ offset }) {
  ...
  return (
    <mesh ref={ref}>
      ...
      <meshBasicMaterial color="#e8ddd0" transparent opacity={0.6} />
    </mesh>
  )
}
      ...
      {slides.map((_, i) => {
        ...
        return (
          <group key={i} position={[x, 0, z]} rotation={[0, -angle, 0]}>
            {isActive ? <Billboard>{nodeInner}</Billboard> : nodeInner}
          </group>
        )
      })}
    </group>
...
function KamadoModel({ current }) {
  ...
  return (
    <primitive ref={ref} object={scene} scale={2} position={[0, 0, 0]} />
  )
}
...
function KamadoScene({ current, onSelect }) {
  return (
    <Canvas
    ...
    >
      ...
      <OrbitalRing current={current} onSelect={onSelect} />
    </Canvas>
  )
}

export default KamadoScene
